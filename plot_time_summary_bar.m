...
for i = 1:numel(method_order)
    ...
    bar(i-0.2,ymean_pp(i),0.3,'FaceColor','none','EdgeColor',plot_color(i,:),'LineWidth',3);
    hold on;
    bar(i+0.2,ymean_vp(i),0.3,'FaceColor',plot_color(i,:),'EdgeColor',plot_color(i,:),'LineWidth',3);
    ...
    if numel(js) > 1
        ...
        plot([i-0.2 i-0.2],[ymean_pp(i) yerr_pp(i)+ymean_pp(i)],'Color',plot_color(i,:)','LineWidth',3);
        plot([i+0.2 i+0.2],[ymean_vp(i) yerr_pp(i)+ymean_vp(i)],'Color',plot_color(i,:)','LineWidth',3);
        plot([i-0.3 i-0.1],[yerr_pp(i)+ymean_pp(i) yerr_pp(i)+ymean_pp(i)],'Color',plot_color(i,:)','LineWidth',3);
        plot([i+0.1 i+0.3],[yerr_pp(i)+ymean_vp(i) yerr_pp(i)+ymean_vp(i)],'Color',plot_color(i,:)','LineWidth',3);
    end
